...
for case in range(number_of_tests):
    ...
    matrix_to_unite_them_all.append(list_matrix)
#End of the input

for small_matrix in matrix_to_unite_them_all:
    list_addresses = [[float('inf') for i in range (number_of_nodes)] for j in range(number_of_nodes)]
    for i in range(number_of_nodes-1):
        from_node = small_matrix[i][0]
        to_node = small_matrix[i][1]
        node_value = small_matrix[i][2]
